...
Results are built in one place for every engine. Each engine reads its driver's rows into engine-neutral `RawNode`/`RawRelationship`/`RawPath` values keyed by database identity (`bolt_rows` in [bolt.py](src/neontology/graphengines/bolt.py), `networkx_rows` in [networkxengine.py](src/neontology/graphengines/networkxengine.py)), and `build_result()` in [hydration.py](src/neontology/graphengines/hydration.py) builds them: each node and relationship once per result, `nodes`/`relationships`/`paths` deduplicated by identity (never by contents), and anything that cannot be built left out with a warning. Engine-specific result handling belongs in the row adapters, never in the builder. Tests are in [tests/test_results.py](tests/test_results.py). [schema_utils.py](src/neontology/schema_utils.py) turns model annotations into `NodeSchema`/`RelationshipSchema` and renders markdown tables (`neontology_schema()` on nodes/relationships). [tools/](src/neontology/tools/) provides bulk ingest from JSON/YAML/Markdown-frontmatter files and from raw record dicts.
