...
- **`export_records()`, `export_yaml()` and `export_json()`, writing a graph back out as content.** A record per node, with each relationship declared under the node it leaves, which is the shape a repository is usually written in - so what comes out looks like what a person would have written, and reads back in. Properties with no value are left out unless `exclude_none=False`.
