...
- **`RELATIONSHIPS_IN` on a node record, for the relationships arriving at it.** The mirror of `RELATIONSHIPS_OUT`: the node declaring them is the target of every one, and the far end is named with `SOURCES`, `SOURCE_NODES` or `SOURCE`. Declaring relationships only where they leave is still the convention worth keeping, but the arriving end is sometimes the one with something to say. A node can declare both blocks, and a record inside either may not name its own end.
- **Both ends of a relationship are now named the same way**: a single value (`SOURCE`/`TARGET`), a list (`SOURCES`/`TARGETS`), or records defining the nodes inline (`SOURCE_NODES`/`TARGET_NODES`). One end names a single node and the other may name several; naming several at both ends is an error, since which relates to which would not be defined.
- **`SOURCE_PROPERTY`, matching the source of a relationship on a property which is not its primary property**, as `TARGET_PROPERTY` already did for the target. Both work in a record of its own and inside either relationship block - including for the declaring node's own end, whose record then has to carry that property.
- **`import_csv()`**, reading a record per row. A row is one flat record, so it can describe a node or a relationship but nothing nested: `RELATIONSHIPS_OUT`, `RELATIONSHIPS_IN`, `SOURCE_NODES` and `TARGET_NODES` are refused with an explanation rather than being misread. Keys which are the same down a whole file can be given once in `defaults` instead of as a column, and a row with a column of its own overrides it. An empty cell means the property was not given, so the model's default applies rather than the property being set to an empty string, and every value is converted by the model - so columns of numbers, booleans and dates arrive as the type the model declares. A row which does not validate is reported with the line it is on.
- **`nested=True` on a result's `neontology_dump()` and `neontology_dump_json()`, for the node oriented shape**: a record per node, each carrying the relationships that leave it, rather than nodes and edges side by side. That is how content is usually written by hand, so a graph can be written back out in the shape a repository takes and read in again. A relationship is only built when the result holds the nodes at both of its ends, so return them alongside it.
- `RecordOrigin` now carries the line a record was on, for formats which are one record per line, and reports it.
